...
import { ArrowLeft, Zap, TrendingUp, Cloud, ExternalLink, Code2, Cpu, CheckCircle2 } from "lucide-react";
    ...
    <group ref={groupRef}>
      <Float speed={2} rotationIntensity={1} floatIntensity={2}>
        <Box args={[1, 1, 1]} position={[-2, 1, -2]} rotation={[0.5, 0.5, 0]}>
          <MeshTransmissionMaterial color="#0ea5e9" transmission={0.9} roughness={0.1} thickness={2} />
        </Box>
      </Float>
      <Float speed={1.5} rotationIntensity={2} floatIntensity={1.5}>
        <Box args={[1.5, 1.5, 1.5]} position={[2, -1, -1]} rotation={[-0.5, 0.2, 0.5]}>
          <MeshTransmissionMaterial color="#38bdf8" transmission={0.9} roughness={0.2} thickness={1.5} />
        </Box>
      </Float>
    </group>
  );
...
export default function AutoCode() {
  ...
  const features = [
    { icon: <Zap className="w-8 h-8" />, title: "Lightning Fast", desc: "40% faster page transitions and 30% lower TTI compared to traditional MVC-based web apps. Every millisecond optimized." },
    { icon: <TrendingUp className="w-8 h-8" />, title: "Real-Time Sync", desc: "50% reduction in data-fetch latency with Convex backend, eliminating client-side state management complexity entirely." },
    { icon: <Cloud className="w-8 h-8" />, title: "Serverless Scale", desc: "Sub-second cold starts and 90+ Lighthouse scores. Containerized with Docker and deployed effortlessly on Google Cloud." },
  ];
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full">
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-sky-400 tracking-widest uppercase font-bold text-sm">Case Study</span>
            </div>
            ...
            <p className="text-xl md:text-2xl text-slate-400 max-w-2xl leading-relaxed mb-10">
              An AI-powered, multi-language online IDE delivering exceptional performance with real-time collaboration capabilities.
            </p>

            <MagneticWrapper className="inline-block" intensity={0.2}>
              <a href="https://auto-code-mu.vercel.app/" target="_blank" rel="noopener noreferrer" 
            ...
            <div className="lg:col-span-4 relative">
              <div className="sticky top-32">
                ...
                <p className="text-lg text-slate-400 mb-8">
                  Built from the ground up to handle intense computational loads while maintaining a buttery-smooth 60fps interface.
                </p>
                <div className="flex flex-wrap gap-2">
                  {["Next.js", "TypeScript", "Convex", "Docker", "GCP"].map(tech => (
                    <span key={tech} className="px-4 py-2 bg-sky-500/10 border border-sky-500/20 text-sky-300 rounded-xl text-sm font-semibold">
                      {tech}
                    </span>
                  ))}
                </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              ...
              <h3 className="text-xl font-bold text-sky-400 mb-2">Lighthouse Score</h3>
              <p className="text-slate-400">Achieved optimal performance metrics across accessibility, best practices, and SEO.</p>
            </motion.div>
